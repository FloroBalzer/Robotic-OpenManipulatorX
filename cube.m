...
if (abs(cube_pos(1)) < 15 && abs(cube_pos(2)) < 15)
    innit_height = 0;
else
    innit_height = cube_height;
end
...
%% rotation
while rot_tracker ~= 0
    if (abs(cube_pos(1)) < 15 && abs(cube_pos(2)) < 15) || too_far == 1 %within limit area, move to rotation location
        point_matrix = [];
